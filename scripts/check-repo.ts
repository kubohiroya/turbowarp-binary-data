...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-Binary-Data') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-binary-data.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-binary-data/issues') {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/binary-data.js')) {
